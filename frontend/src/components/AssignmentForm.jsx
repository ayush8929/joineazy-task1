...
export default function AssignmentForm({ groups, onCreated }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      await assignmentsApi.create({
        ...
        group_ids: targetType === 'group' ? selectedGroups : undefined,
      });
      setTitle('');
      setDescription('');
      setDueDate('');
    ...
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      {targetType === 'group' && (
        <div className="border border-slate-200 rounded-md p-3 max-h-32 overflow-y-auto space-y-1">
          {groups.length === 0 && <p className="text-xs text-slate-400">No groups exist yet.</p>}
          {groups.map((g) => (
